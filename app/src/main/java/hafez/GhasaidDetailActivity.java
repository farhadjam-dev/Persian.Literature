...
public class GhasaidDetailActivity extends AppCompatActivity {
    ...
    // نگاشت عنوان قصیده به لینک صوتی Google Drive (فعلاً خالی)
    private static final Map<String, String> AUDIO_URLS = new HashMap<String, String>() {{
        //put("قصیده شماره ۱", "لینک صوتی"); // بعداً لینک‌ها رو اضافه کنید
    }};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // دریافت عنوان قصیده از Intent
        ghasaidTitle = getIntent().getStringExtra("ghasaidTitle");
        if (ghasaidTitle != null) {
            toolbarTitle.setText(ghasaidTitle);
        }

        // بارگذاری ابیات قصیده
        List<Verse> verseList = loadVersesFromJson(ghasaidTitle);
        ...
        // بررسی وضعیت علاقه‌مندی برای قصیده فعلی
        isFavorite = mutableFavorites.contains(ghasaidTitle);
        ...
        favoriteButton.setOnClickListener(v -> {
            ...
            if (isFavorite) {
                isFavorite = false;
                mutableFavorites.remove(ghasaidTitle);
                ...
                isFavorite = true;
                mutableFavorites.add(ghasaidTitle);
    ...
    private List<Verse> loadVersesFromJson(String ghasaidTitle) {
