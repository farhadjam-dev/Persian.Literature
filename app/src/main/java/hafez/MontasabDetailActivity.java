...
public class MontasabDetailActivity extends AppCompatActivity {
    ...
    // نگاشت عنوان شعر منتسب به لینک صوتی Google Drive (فعلاً خالی)
    private static final Map<String, String> AUDIO_URLS = new HashMap<String, String>() {{
        //put("شعر منتسب شماره ۱", "لینک صوتی"); // بعداً لینک‌ها رو اضافه کنید
    }};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_montasab_detail);
        ...
        // دریافت عنوان شعر منتسب از Intent
        montasabTitle = getIntent().getStringExtra("montasabTitle");
        if (montasabTitle != null) {
            toolbarTitle.setText(montasabTitle);
        }

        // بارگذاری ابیات شعر منتسب
        List<Verse> verseList = loadVersesFromJson(montasabTitle);
        ...
        // بررسی وضعیت علاقه‌مندی برای شعر منتسب فعلی
        isFavorite = mutableFavorites.contains(montasabTitle);
        ...
        favoriteButton.setOnClickListener(v -> {
            ...
            if (isFavorite) {
                isFavorite = false;
                mutableFavorites.remove(montasabTitle);
                ...
                isFavorite = true;
                mutableFavorites.add(montasabTitle);
    ...
    private void playAudio() {
        String audioUrl = AUDIO_URLS.get(montasabTitle);
        if (audioUrl == null) {
            Toast.makeText(this, "فایل صوتی برای این شعر منتسب یافت نشد", Toast.LENGTH_SHORT).show();
            return;
        }

        File cacheFile = new File(getCacheDir(), montasabTitle.replace(" ", "_") + ".mp3");
    ...
    private List<Verse> loadVersesFromJson(String montasabTitle) {
