...
public class SaadiRubaiyatDetailActivity extends AppCompatActivity {
    ...
    private String saadiRubaiyatTitle;
    private MediaPlayer mediaPlayer;
    private Handler handler = new Handler(Looper.getMainLooper());
    private boolean isPlaying = false;

    // نگاشت عنوان رباعی به لینک صوتی (مثال)
    private static final Map<String, String> AUDIO_URLS = new HashMap<String, String>() {{
        // بعداً لینک‌های واقعی اضافه کنید
        put("رباعی شماره ۱ سعدی", "https://example.com/saadi_rubaiyat_1.mp3");
        put("رباعی شماره ۲ سعدی", "https://example.com/saadi_rubaiyat_2.mp3");
    }};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_saadi_rubaiyat_detail);
        ...
        saadiRubaiyatTitle = getIntent().getStringExtra("saadiRubaiyatTitle");
        if (saadiRubaiyatTitle != null) {
            toolbarTitle.setText(saadiRubaiyatTitle);
        }

        List<Verse> verseList = loadVersesFromJson(saadiRubaiyatTitle);
        ...
        isFavorite = mutableFavorites.contains(saadiRubaiyatTitle);
        ...
        favoriteButton.setOnClickListener(v -> {
            ...
            if (isFavorite) {
                isFavorite = false;
                mutableFavorites.remove(saadiRubaiyatTitle);
                ...
                isFavorite = true;
                mutableFavorites.add(saadiRubaiyatTitle);
    ...
    private void playAudio() {
        String audioUrl = AUDIO_URLS.get(saadiRubaiyatTitle);
        if (audioUrl == null) {
            Toast.makeText(this, "فایل صوتی برای این رباعی یافت نشد", Toast.LENGTH_SHORT).show();
            return;
        }

        File cacheFile = new File(getCacheDir(), saadiRubaiyatTitle.replace(" ", "_") + ".mp3");
    ...
    private List<Verse> loadVersesFromJson(String saadiRubaiyatTitle) {
        List<Verse> verseList = new ArrayList<>();
        try {
            InputStream inputStream = getResources().openRawResource(R.raw.saadi_rubaiyat);
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if (title.equals(saadiRubaiyatTitle)) {
