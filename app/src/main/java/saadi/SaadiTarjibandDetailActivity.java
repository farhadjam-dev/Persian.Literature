...
public class SaadiTarjibandDetailActivity extends AppCompatActivity {
    ...
    private String saadiTarjibandTitle;
    private MediaPlayer mediaPlayer;
    private Handler handler = new Handler(Looper.getMainLooper());
    private boolean isPlaying = false;

    // نگاشت عنوان ترجیع‌بند به لینک صوتی (مثال)
    private static final Map<String, String> AUDIO_URLS = new HashMap<String, String>() {{
        // بعداً لینک واقعی اضافه کنید
        put("ترجیع‌بند سعدی", "https://example.com/saadi_tarjiband.mp3");
    }};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_saadi_tarjiband_detail);
        ...
        saadiTarjibandTitle = getIntent().getStringExtra("saadiTarjibandTitle");
        if (saadiTarjibandTitle != null) {
            toolbarTitle.setText(saadiTarjibandTitle);
        } else {
            saadiTarjibandTitle = "ترجیع‌بند سعدی"; // مقدار پیش‌فرض
            toolbarTitle.setText(saadiTarjibandTitle);
        }

        List<Verse> verseList = loadVersesFromJson(saadiTarjibandTitle);
        ...
        isFavorite = mutableFavorites.contains(saadiTarjibandTitle);
        ...
        favoriteButton.setOnClickListener(v -> {
            ...
            if (isFavorite) {
                isFavorite = false;
                mutableFavorites.remove(saadiTarjibandTitle);
                ...
                isFavorite = true;
                mutableFavorites.add(saadiTarjibandTitle);
    ...
    private void playAudio() {
        String audioUrl = AUDIO_URLS.get(saadiTarjibandTitle);
        if (audioUrl == null) {
            Toast.makeText(this, "فایل صوتی برای این ترجیع‌بند یافت نشد", Toast.LENGTH_SHORT).show();
            return;
        }

        File cacheFile = new File(getCacheDir(), saadiTarjibandTitle.replace(" ", "_") + ".mp3");
    ...
    private List<Verse> loadVersesFromJson(String saadiTarjibandTitle) {
        List<Verse> verseList = new ArrayList<>();
        try {
            InputStream inputStream = getResources().openRawResource(R.raw.saadi_tarjiband);
            ...
            // چون فقط یه ترجیع‌بند داریم، اولین شیء رو می‌گیره
            ...
            e.printStackTrace();
            Toast.makeText(this, "خطا در بارگذاری ترجیع‌بند", Toast.LENGTH_SHORT).show();
        }
        return verseList;
    }
